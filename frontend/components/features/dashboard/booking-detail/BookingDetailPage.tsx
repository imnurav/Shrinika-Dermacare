'use client';
import { useBookingDetailPage } from '@/lib/hooks/dashboard/useBookingDetailPage';
...
export default function BookingDetailPage({ bookingId, initialBooking }: Props) {
  const { booking, error, setError, totalAmount } = useBookingDetailPage(bookingId, initialBooking);
    ...
    <div className="flex h-full min-h-0 flex-col gap-4">
      <PageHeader
        ...
        breadcrumbs={[
          ...
          { label: bookingId ? `Booking ${bookingId.slice(0, 8)}` : 'Booking Detail' },
        ]}
        actionButton={<ActionButton variant="primary" onClick={() => window.print()}>
